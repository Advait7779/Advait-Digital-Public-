...
with Image.open(img_path) as img:
    ...
    # Let's find the bounding box of the circular icon on the left (x < 80)
    min_x, max_x = width, 0
    ...
    print(f"Icon bounding box: x in [{min_x}, {max_x}], y in [{min_y}, {max_y}]")
    
    # We crop with 2 pixels padding if possible
    pad = 2
    crop_x1 = max(0, min_x - pad)
    crop_y1 = max(0, min_y - pad)
    crop_x2 = min(80, max_x + pad)
    crop_y2 = min(height, max_y + pad)
    
    icon_w = crop_x2 - crop_x1
    icon_h = crop_y2 - crop_y1
    
    # Make it a perfect square by padding the smaller dimension
    if icon_w > icon_h:
        diff = icon_w - icon_h
        ...
        crop_x2 = min(80, crop_x2 + (diff - diff // 2))
